            ...
            >
                <div class="flex flex-col">
                    <input
                        ...
                        class="p-2 m-1"
                        :class="updateStampsErrors[`${grading.id}.abbreviation`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="updateStampsErrors[`${grading.id}.abbreviation`]">{{ updateStampsErrors[`${grading.id}.abbreviation`][0] }}</span>
                </div>
                <div class="flex flex-col">
                    <input
                        type="text"
                        ...
                        placeholder="Grading"
                        class="p-2 m-1"
                        :class="updateStampsErrors[`${grading.id}.grading`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="updateStampsErrors[`${grading.id}.grading`]">{{ updateStampsErrors[`${grading.id}.grading`][0] }}</span>
                </div>
                <div class="flex flex-col w-full">
                    <input
                        ...
                        :class="updateStampsErrors[`${grading.id}.description`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="updateStampsErrors[`${grading.id}.description`]">{{ updateStampsErrors[`${grading.id}.description`][0] }}</span>
                </div>
            ...
            >
                <div class="flex flex-col">
                    <input
                        ...
                        placeholder="Abbreviation"
                        class="p-2 m-1"
                        :class="addStampsErrors[`${index}.abbreviation`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="addStampsErrors[`${index}.abbreviation`]">{{ addStampsErrors[`${index}.abbreviation`][0] }}</span>
                </div>
                <div class="flex flex-col">
                    <input
                        type="text"
                        ...
                        placeholder="Grading"
                        class="p-2 m-1"
                        :class="addStampsErrors[`${index}.grading`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="addStampsErrors[`${index}.grading`]">{{ addStampsErrors[`${index}.grading`][0] }}</span>
                </div>
                <div class="flex flex-col w-full">
                    <input
                        type="text"
                        ...
                        placeholder="Description"
                        class="p-2 m-1 w-full"
                        :class="addStampsErrors[`${index}.description`] ? 'border-red-500 border-2' : 'border border-highlight'"
                    >
                    <span class="flex-1 p-2 italic text-xs font-bold text-red-500" v-if="addStampsErrors[`${index}.description`]">{{ addStampsErrors[`${index}.description`][0] }}</span>
                </div>
        ...
        storeGradings() {
            axios.post('/admin/gradings', {gradings: this.addGradings})
                .then(response => {
                    ...
                    this.addStampsErrors = {};
                    this.showAddedMessage = true;
                })
                .catch(error => {
                    console.log(error);
                    this.addStampsErrors = error.response.data.errors;
                });
        },
        updateGradings() {
            axios.post('/admin/gradings', {
                ...
                .then(response => {
                    this.updateStampsErrors = {};
                    this.showUpdatedMessage = true;
                })
                .catch(error => {
                    console.log(error);
                    this.updateStampsErrors = error.response.data.errors;
                });
        },
