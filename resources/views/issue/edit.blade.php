...
@section('content')
    ...
    <form method="POST" action="{{ route('update.issue', ['issue' => $issue]) }}">
        ...
        <div class="flex items-center mb-6">
            ...
            <div class="flex flex-col w-2/3">
                <textarea id="description" name="description" placeholder="Description..." class="w-full p-2 rounded border shadow @error('description') border-red-500 @enderror">{{ old('description', $issue->description) }}</textarea>
                @error('description')
                    @component('components.error') {{ $message }} @endcomponent
                @enderror
            </div>
