...
@section('content')
    <div class="flex justify-between items-center">
        ...
        @endisset
        @can('update issue')
            <a href="{{ route('issue.edit', ['issue' => $issue]) }}" class="bg-purple-500 text-white border rounded px-4 py-2">
            ...
            <form method="POST" action="{{ route('issue.delete', ['issue' => $issue]) }}" onsubmit="return confirm('Are you sure you want to delete this issue?')">
                @csrf
                @method('DELETE')
    ...
    <div class="flex flex-col">
        ...
        <div class="flex justify-between items-center">
            ...
            @can('create stamp')
                <a href="{{ route('stamp.create', ['issue' => $issue]) }}" class="flex bg-green-500 text-white border rounded px-4 py-2">
                ...
                <div class="flex flex-col w-1/4 items-center justify-center p-1 mr-2">
                    ...
                    @auth
                        {{-- @if (!$collection->contains($stamp)) --}}
                            ...
                            <a href="{{ route('collection.show', ['stamp' => $stamp, 'slug' => $stamp->slug]) }}" class="border rounded p-2 text-center w-full bg-blue-400 text-white">Manage Collection</a>
                        {{-- @else
                        ...
                        @can('update stamp')
                            <div class="flex mt-3">
                                <a href="{{ route('stamp.edit', ['stamp' => $stamp]) }}" class="bg-purple-500 text-white border rounded px-4 py-2">
                                    <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="20" height="24" viewBox="0 0 20 20">
                                        <path d="M12.3 3.7l4 4L4 20H0v-4L12.3 3.7zm1.4-1.4L16 0l4 4-2.3 2.3-4-4z"></path>
                                    </svg>
                                </a>
                                <form method="POST" action="{{ route('stamp.delete', ['stamp' => $stamp]) }}" onsubmit="return confirm('Are you sure you want to delete this stamp?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white border rounded px-4 py-2">
                                        <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
